...
namespace KalkulatorRatalnyMAUI
{
    ...
    public partial class HarmonogramPage : ContentPage
    {
        ...
        // Obsługa zapisu harmonogramu do pliku PDF (wywołuje metodę generującą PDF asynchronicznie)
        private async void OnSavePdfClicked(object sender, EventArgs e)
        {
                ...
                try
                {
                    ...
                    await DisplayAlert("Sukces", $"Plik PDF został zapisany:\n{filePath}", "OK");
                }
                catch (Exception ex)
                {
                    await DisplayAlert("Błąd", $"Nie udało się wygenerować PDF:\n{ex.Message}", "OK");
                }
            #else
                await DisplayAlert("Informacja", "Generowanie PDF jest obecnie obsługiwane tylko w systemie Windows.", "OK");
            #endif
        }
